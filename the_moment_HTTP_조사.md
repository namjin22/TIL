...
#### 무상태
- 이전 요청과 다음 요청 사이의 상태(연결 정보)를 저장하지 않는 Stateless(무상태) 특성으로 클라이언트가 요청할 때마다 모든 정보를 함께 보내야 하므로 서버는 각 요청을 독립적으로 처리할 수 있고, 요청 간 상태를 저장하지 않아 서버의 부하가 줄어드는 장점이 있다.

- 하지만 반대로 무상태 특성은은 로그인 상태 유지나 장바구니처럼 '상태'를 기억해야 하는 기능 구현은 어려워지는 단점도 있다.
...
##### URL 길이 제한
실제로 URL 길이는 브라우저나 서버에 따라 제한이 있으며, 일반적으로는 약 2000~8000자 사이에서 제한된다.

Internet Explorer는 약 2048자, Chrome과 Firefox는 8000자 이상을 지원하지만,

서버가 더 짧은 길이로 URL을 제한할 수도 있어 너무 긴 데이터를 GET 요청으로 보내면 잘릴 수 있는 위험이 있다.
...
### HTTP의 메서드들(GET, POST 등)은 `멱등성`, `안전성`, `캐시 가능성` 등의 특징을 가진다.

- 멱등성
  - 같은 요청을 여러 번 보내도 결과가 같음
  - ex : `DELETE /item/3`을 여러 번 보내도 결과는 항상 삭제됨 상태 유지.

- 안전성
  - 서버 상태를 바꾸지 않음
  - ex : GET은 데이터를 조회만 하므로 안전성을 가짐.

- 캐시 가능성
  - 응답을 저장해두고 재사용 가능함
  - ex : GET /news/1은 서버 응답을 캐싱해 재사용 가능.

예시를 적어 둔 것처럼 각 메서드들에 따라 `멱등성`, `안전성`, `캐시 가능성` 등의 특징을 가진다.
...
`Host: example.com`은 요청 대상 서버가 example.com이라는 걸, `User-Agent: Mozilla/5.0`은은 클라이언트 정보를, `Content-Type: application/json`은 보낼 데이터의 형식을 알려준다.
...
요청 구조와 비슷하게 각 문단마다 응답 데이터 형식, 캐시 전략, 쿠키 설정 정보를 알려준다.
