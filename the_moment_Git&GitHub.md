...
- `*gitignore*`
    ...
    - gitignore은 Git이 추적하지 않아야 할 파일/폴더를 정리해둔 텍스트 파일이다.

    - Git은 .gitignore에 있는 파일을 추적하지 않는다. (add, commit 등에 제외됨)

    - 프로젝트 루트에 `.gitignore` 파일을 만들어 사용함.
  

            *.log // 모든 .log 파일 무시

            build/ // build 폴더 전체 무시

            secret.txt // secret.txt 파일 무시
...
- `remote`
    ...
    - `git remote`는 로컬 저장소와 연결된 원격 저장소 정보를 관리하는 명령어이다.

원격 저장소 목록 확인은 : `git remote -v`

원격 저장소 추가는 : `git remote add origin <저장소 URL>`

원격 저장소 삭제는 : `git remote remove origin`
을 입력하여 사용한다.
...
- `*switch*`
  ...
  - `switch`는 Git 버전 2.23 이상부터 도입되었다.

  - `checkout`보다 더 명확하고 안전하게 브랜치 작업을 하기 좋다.

  - 기존 브랜치 전환할 때는 : `git switch 브랜치명`

  - 새 브랜치 생성 및 전환할 때는 : `git switch -c 새브랜치명`

        checkout은 브랜치 전환 외에도 파일 복원 등 다양한 기능을 포함해서 약간 헷갈릴 수 있었는데,
        
        switch는 브랜치 전환에만 집중되어 있다.
...
- Git 충돌(conflict)
    ...
    - 두 브랜치에서 같은 파일의 같은 부분이 서로 다르게 수정된 경우 `git merge` 시 충돌이 발생합니다.

    - 충돌이 발생하면 개발자가 수동으로 수정 후 커밋해야 합니다.

  - #### 충돌 해결 방법

  1. 충돌이 난 파일을 열면 Git이 구분 표시(`<<<<<<<`, `=======`, `>>>>>>>`)를 추가해줍니다.

  2. 어느 쪽 코드를 사용할지 수동으로 선택 후 저장합니다.

  3. `git add` 후후 `git commit`을 통해 충돌 해결 완료.
